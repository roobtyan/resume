...
\begin{itemize}
  ...
  \item Created SonarQube plugin for the pinpoint analyzer and Google\textquotesingle s findbugs analyzer
\end{itemize}
...
\begin{itemize}
  \item Built-in Java/Kotlin highlightings and completions, other languages can be supported via plugins. Has extra build \& run support for Kotlin.
  \item Flexible architecture, high-level logic is decoupled with UI framework, easy to migrate to other UI frameworks.
  \item JetBrains IDE plugins can be easily transplanted to this IDE.
  \item Provide fine-grained highlight color and keymap settings, settings can be hot reloaded.
\end{itemize}
...
\begin{itemize}
  \item Supports lambdas and lazy (call by need) / macro (call by name) / strict (call by value).
  ...
  \item Provides JetBrains IDE plugin supporting semantics-based highlightings, error checkings, completions, jump-to-definition, renaming, eval and replace, quick fixes, etc.
  \item Provides a command-line REPL with GHCi-style code completion and colored output.
\end{itemize}
...
\begin{itemize}[parsep=0.5ex]
  ...
  \item StackOverflow: \url{http://stackoverflow.com/users/7083401/ice1000} \\
    moderator with some privileges, also active on \href{https://stackexchange.com/users/9532102/ice1000} {other StackExchange} sites
